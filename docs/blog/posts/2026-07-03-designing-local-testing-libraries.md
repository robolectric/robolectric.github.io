...
([`frameworks/support`][frameworks-support]) tree, linked inline.
...
a device or emulator ([Fundamentals of testing Android apps][fundamentals]). Local tests are fast and hermetic; the guidance
is to reserve device tests for "cases where you must test against the behavior of a real device"
([Instrumented tests][instrumented-tests]). A platform with no local-testing story pushes every
consumer onto the slow, flaky end of that tradeoff.
...
real device, just in-process and fast ([Robolectric architecture][architecture]). You choose the SDK level with
`@Config` and can switch on real graphics for screenshots ([Configuring Robolectric][configuring]). That is the
...
instrumented, because the runner delegates to Robolectric's test runner in local runs
([AndroidJUnit4][androidjunit4]; [Robolectric strategies][strategies]). So a good test library
never chooses "Robolectric or device". It targets that unified runner and runs in both. Robolectric
is best thought of as a deployment target for your tests, not a separate code path.
...
on the JVM from one binary, with no Robolectric-specific source set. The only runtime detection is a
...
assertThat(workManager.getWorkInfoById(id).get().getState().isFinished()).isFalse();
mTestDriver.setAllConstraintsMet(id);                                  // the lever
assertThat(workManager.getWorkInfoById(id).get().getState().isFinished()).isTrue();  // ran inline
```
...
returns the doubles ([`TestCarContext.java`][testcarcontext]). The trick is faking the *callback*
direction so output is assertable ([`FakeHost.java`][fakehost]):
...
[frameworks-support]: https://cs.android.com/androidx/platform/frameworks/support
...
[androidjunit4]: https://developer.android.com/reference/androidx/test/ext/junit/runners/AndroidJUnit4
...
[graphicsmode]: https://robolectric.org/javadoc/latest/org/robolectric/annotation/GraphicsMode.html
...
[fakehost]: https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:car/app/app-testing/src/main/java/androidx/car/app/testing/FakeHost.java
